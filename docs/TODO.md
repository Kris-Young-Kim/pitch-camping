...
- [ ] 반려동물 동반 여행지 필터링
  - 필터 옵션 추가: "반려동물 동반 가능" 체크박스
  - 키워드 검색에 "반려동물", "펫", "애완동물" 등 자동 포함 옵션
  - 여행지 타입별 필터링 (숙박, 관광지, 음식점 등)
- [ ] 반려동물 동반 여행지 정보 표시
  - 여행지 카드에 반려동물 동반 가능 뱃지 표시
  - 상세페이지에 반려동물 동반 정보 섹션 추가
  - 반려동물 시설 정보 표시 (펜션, 호텔, 카페 등)
  - 반려동물 규정 및 주의사항 표시
- [ ] 반려동물 동반 여행지 데이터베이스 스키마
  - travels 테이블에 반려동물 동반 관련 필드 추가
  - pet_friendly 테이블 생성 (반려동물 동반 여행지 정보)
  - 반려동물 시설 정보 저장 (펜션, 호텔, 카페 등)
  - 반려동물 규정 및 주의사항 저장
- [ ] 반려동물 동반 여행 가이드/정보
  - 반려동물 동반 여행 가이드 페이지 (`app/pet-travel-guide/page.tsx`)
  - 반려동물 동반 여행 시 주의사항 안내
  - 지역별 반려동물 동반 가능 여행지 추천
  - 반려동물 동반 여행 팁 및 체크리스트
- [ ] 반려동물 동반 여행지 리뷰/평점
  - 리뷰에 반려동물 동반 경험 포함
  - 반려동물 동반 만족도 별도 평가
  - 반려동물 동반 리뷰 필터링 및 검색
- [ ] 반려동물 동반 여행지 지도 표시
  - 지도에서 반려동물 동반 가능 여행지 마커 구분
  - 반려동물 동반 여행지만 필터링하여 지도 표시
  - 반려동물 동반 여행 경로 계획 기능
...
- [ ] 반려동물 동반 여행 기능 구현 (Phase 2)
  - [ ] 반려동물 동반 여행지 필터 컴포넌트 (`components/travel-filters.tsx`)
    - "반려동물 동반 가능" 필터 옵션 추가
    - 필터 상태 URL 쿼리 파라미터 연동
  - [ ] 반려동물 동반 여행지 뱃지 (`components/travel-card.tsx`, `components/travel-detail/pet-friendly-badge.tsx`)
    - 여행지 카드에 반려동물 동반 가능 뱃지 표시
    - 상세페이지에 반려동물 동반 정보 섹션 추가
  ...
  - [ ] 반려동물 동반 여행 가이드 페이지 (`app/pet-travel-guide/page.tsx`)
    ...
    - 주의사항 및 체크리스트
    - 지역별 반려동물 동반 가능 여행지 추천
  - [ ] 반려동물 동반 여행지 데이터베이스 스키마 (`supabase/migrations/`)
    - travels 테이블에 pet_friendly 필드 추가
    - pet_friendly_info 테이블 생성 (상세 정보)
    - 반려동물 시설 정보 저장 구조 설계
  - [ ] 반려동물 동반 여행지 API 연동
    - TourAPI 키워드 검색에 반려동물 관련 키워드 포함
    - Supabase에서 반려동물 동반 여행지 필터링
    - 반려동물 동반 여행지 상세 정보 조회
